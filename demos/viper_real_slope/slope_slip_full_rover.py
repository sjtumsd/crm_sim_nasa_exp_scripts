...
result_name="FSI_Viper_RealSlope_SlopeAngle"
...
for k in range(1):
    ...
    for i in range(7):
        if i < 7:
            file=open(result_name+"_"+str(i*5)+"/body_position.txt","r")
        else:
            file=open(result_name+"_"+str(i*5)+"/body_position.txt","r")
        Time=[]
        vel=[]
        ...
        slip.append(1 - sumsum/numnum/0.8/0.25)
        if i < 7:
            slope.append(i*5.0)
        else:
...
plt.savefig("./FullViperRover_73kg_GRC3.png", dpi=300)